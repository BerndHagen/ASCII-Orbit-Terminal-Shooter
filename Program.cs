    ...
    class Program
    {
        ...
        static void MoveInvaders()
        {
            for (int i = Invaders.Count - 1; i >= 0; i--)
            {
                ...
                ClearPosition(AreaLeft + invader.PositionX, AreaTop + invader.PositionY);
                invader.Move(Level);
                if (invader.PositionY >= GameHeight - 2)
                {
                    // Beep once when an invader reaches the bottom border
        ...
        {
            int baseScore = 1000;
            int rankIncrement = 2500;
            int rank = (Score - baseScore) / rankIncrement;
            if (Score < baseScore) return "F";
            else if (rank == 0) return "F";
